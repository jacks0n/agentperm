...
from .shell_tokens import (
    validate_flag_name as validate_flag_name,
)
...
def parse_shell_pattern(pattern: str) -> ShellPattern:
    ...
    for token in tokens:
        ...
        if remainder.startswith("sqlvalues(") and remainder.endswith(")"):
            ...
            members = split_set_members(remainder[10:-1])
        ...
        if is_flag(remainder):
            ...
            if "=" in remainder:
                ...
                value_glob = to_glob(remainder[eq_pos + 1 :])
        ...
        if remainder.startswith("{") and remainder.endswith("}"):
            ...
            if is_all_flags:
                ...
                globs = tuple(to_glob(m) for m in members)
        ...
        globs = expand_positional_alternation(remainder)
    ...
    *,
    ...
    def short_value_prefix(arg: str) -> str | None:
        ...
        candidates = (
            ...
            if atom.startswith("-") and not atom.startswith("--") and len(arg) > len(atom) and arg.startswith(atom)
        )
...
def _match_path(
    ...
    def rec(pi: int, oi: int) -> frozenset[int]:
        ...
        if pi >= len(path):
            result: frozenset[int] = rec(pi, oi + 1) if oi in optional_operands else frozenset({oi})
            memo[key] = result
            return result

        term = path[pi]
        skipped: frozenset[int] = rec(pi, oi + 1) if oi in optional_operands else frozenset()
        ...
        if isinstance(term, (SqlCapture, NestedShellCapture)):
            matched: frozenset[int] = rec(pi + 1, oi + 1)
...
def match_shell_pattern(
    ...
    *,
    ...
    preserve_double_dash = any(isinstance(term, Word) and term.glob == "--" for term in pattern.path)
    ...
    for c in pattern.flags:
        if c.disp is Disposition.Required:
            ...
            if c.value_glob is not None:
                ...
                if vals is None or not all(v is not None and fnmatch.fnmatch(v, c.value_glob) for v in vals):
                    return False
