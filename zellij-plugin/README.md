# agentperm-indicator (zellij plugin)

A small zellij WASM plugin that lets you toggle agentperm "bypass everything"
...
When the flag is on, agentperm coerces every `Ask` and `NoOpinion` verdict to
...
plugin needs it to read and write `~/.cache/agentperm/bypass/<session>/`.
...
3. `ls ~/.cache/agentperm/bypass/$ZELLIJ_SESSION_NAME/` shows a file named
   `$ZELLIJ_PANE_ID`. (Permissions follow your umask — typically `-rw-r--r--`.
   agentperm only refuses group/world-*writable* directories, so the default
   umask of 022 is safe; tighten with `chmod -R go-rwx ~/.cache/agentperm` if
...
## How it talks to agentperm

Both sides resolve the cache dir the same way:
`$XDG_CACHE_HOME/agentperm/bypass/<session>/<pane_id>` (falling back to
`$HOME/.cache/...` if `XDG_CACHE_HOME` is unset).

agentperm reads the flag file inside `coerce_for_pane_bypass`. The plugin owns
all writes. agentperm refuses to honor the flag if the bypass directory is
...
agentperm cannot be retroactively unapproved by toggling bypass off mid-flight.
